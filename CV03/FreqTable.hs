...
-- najdi n najcastejsie vyskytujucich sa slov v Stringu
-- vo vysledku su utriedene od najmenej casteho po najviac
-- ak su rovnako casto, tak abecedne
-- mostFrequent 5 hamlet
...
-- zatvorkova verzia pre Lispistov
mostFrequentMW n ws = tail ( 
                        concat (  
                          map (\(c,w)->" " ++ w) ( 
                            reverse ( 
                              take n ( 
...
splitWords ws = filter (/= []) . splitWords' ws
