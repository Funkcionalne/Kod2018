...
main = do
	...
	runTestTT $ 
		TestList [ 
					TestCase $ assertEqual ("splitWords (ch -> ord ch < 64) myhamlet ") (length (mysplitWords (\ch -> ord ch < 64) myhamlet)) (length (F.splitWords (\ch -> ord ch < 64) myhamlet)),
					TestCase $ assertEqual ("splitWords (ch -> ch ==  ) myhamlet ") (length (mysplitWords (\ch -> ch == ' ') myhamlet)) (length (F.splitWords (\ch -> ch == ' ') myhamlet)),
					TestCase $ assertEqual ("splitWords (ch -> (elem ch ,.; )) myhamlet ") (length (mysplitWords (\ch -> (elem ch ",.; ")) myhamlet)) (length (F.splitWords (\ch -> (elem ch ",.; ")) myhamlet))
		]
		
		

mysplitWords  :: (Char -> Bool) -> String -> [String]
mysplitWords ws = filter (/= []) . splitWords' ws

splitWords'  :: (Char -> Bool) -> String -> [String]
splitWords' _ []   = []
splitWords' ws xs   =
    (takeWhile (not . ws) (dropWhile ws xs)) :
    (splitWords' ws (dropWhile (not . ws) ( dropWhile ws xs)))

myhamlet = "There was this king sitting in his garden all alane " ++
  ...
  "myhamlet, that's the boy, puts on he's round the bend. " ++
  ...
  " " ++
      "myhamlet! myhamlet! Loved his mommy! " ++
      "myhamlet! myhamlet! Acting barmy! " ++
      "myhamlet! myhamlet! Hesitatin', " ++
      "Wonders if the ghost's a cheat " ++
      "And that is why he's waitin'. " ++
  " " ++
  "Then myhamlet wrote a scene for the players to enact " ++
  ...
  " " ++
  "So myhamlet's got the proof that Claudius gave his dad the dose. " ++
  "The only problem being, now, that Claudius knows he knows. " ++
  "So while myhamlet tells his mother her new husband's not a fit one, " ++
  "Uncle Claude puts out a contract with the English king as hit man. " ++
  " " ++
  "And when myhamlet killed Polonius, the concealed corpus delecti, " ++
  ...
  "myhamlet jumped the boat and put the finger on that pair. " ++
  " " ++
  ...
  " " ++
      "myhamlet! myhamlet! Nae messin'! " ++
      "myhamlet! myhamlet! Learned his lesson! " ++
      "myhamlet! myhamlet! Yorick's crust " ++
  ...
  "With a blunted sword for myhamlet, and a sharpened sword for Leartes. " ++
  " " ++
  "To make things double sure, the old belt and braces line, " ++
  "He fixed a poison sword tip, and a poison cup of wine. " ++
  "The poison sword got myhamlet, but Leartes went and muffed it, " ++
  "'Cause he got stabbed himself, and he confessed before he snuffed it. " ++
  " " ++
  "Then myhamlet's mommy drank the wine, and as her face turned blue, " ++
  "myhamlet said I think the king's a baddie through and through."  ++
  ...
  " " ++
      "myhamlet! myhamlet! Oh so gory! " ++
      "myhamlet! myhamlet! End of story! " ++
      "myhamlet! myhamlet! I'm away! " ++
