...
data Stmt = Nop | String := Expr | If Expr Stmt Stmt | While Expr Stmt | Seq [Stmt] | For String Expr Expr Expr Stmt   deriving Show
...
def = emptyDef{ commentStart = "{-"
              ...
              , opLetter = oneOf "~&=:|"
              , reservedOpNames = ["~", "&", "=", "||", ":="]
              , reservedNames = ["true", "false", "nop", "for",
                                 "if", "then", "else", "fi",
                                 "while", "do", "od"]
              }
...
t2 = "for i:=x; x; x do if x = true then r := x else q := true fi od"        
